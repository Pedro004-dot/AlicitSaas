...
class CompanyService:
    ...
    def get_all_companies(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """
    ...
    def search_companies_by_name(self, search_term: str, limit: int = 50) -> List[Dict[str, Any]]:
        """Buscar empresas por nome"""
        try:
            companies = self.company_repo.search_by_name(search_term, limit)
            return self._format_companies_for_frontend(companies)
        except Exception as e:
            logger.error(f"Erro ao buscar empresas por nome '{search_term}': {e}")
            return []
    
    def search_companies_by_keywords(self, keywords: List[str], limit: int = 50) -> List[Dict[str, Any]]:
        """Buscar empresas por palavras-chave"""
        try:
            companies = self.company_repo.search_by_keywords(keywords, limit)
            return self._format_companies_for_frontend(companies)
        except Exception as e:
