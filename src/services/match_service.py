...
class MatchService:
    ...
    def get_match_by_id(self, match_id: str) -> Optional[Dict[str, Any]]:
        """Buscar match por ID com formatação"""
        try:
    ...
    def get_matches_by_company(self, company_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        """
        Obter matches de uma empresa específica com detalhes
        """
        try:
            matches = self.match_repo.find_matches_by_company_with_details(company_id, limit)
            return self._format_matches_for_frontend(matches)
        except Exception as e:
    ...
    def get_high_score_matches(self, min_score: float = 0.8, limit: int = 50) -> List[Dict[str, Any]]:
        """
        Buscar matches com score alto
        """
        try:
            matches = self.match_repo.find_high_score_matches(min_score, limit)
            return self._format_matches_for_frontend(matches)
        except Exception as e:
