...
class MatchRepository(BaseRepository):
    ...
    def find_by_company_id(self, empresa_id: str, limit: int = 100) -> List[Dict[str, Any]]:
        """Buscar matches por ID da empresa"""
    ...
    def find_high_score_matches(self, min_score: float = 0.8, limit: int = 50) -> List[Dict[str, Any]]:
        """Buscar matches com score alto"""
        query = """
            SELECT * FROM matches 
            WHERE score_similaridade >= %s
            ORDER BY score_similaridade DESC, created_at DESC
            LIMIT %s
        """
        return self.execute_custom_query(query, (min_score, limit))
    
    def find_matches_with_details(self, limit: int = 100) -> List[Dict[str, Any]]:
        """Buscar matches com detalhes de empresa e licitação"""
        query = """
            SELECT 
                m.*,
    ...
    def find_matches_by_company_with_details(self, empresa_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        """Buscar matches de uma empresa com detalhes da licitação"""
        query = """
            SELECT 
                m.*,
                l.objeto_compra,
            ...
            LIMIT %s
        """
        return self.execute_custom_query(query, (empresa_id, limit))
    ...
    def find_grouped_by_company(self) -> List[Dict[str, Any]]:
        """Buscar matches agrupados por empresa (equivalente ao get_companies_with_matches_summary)"""
        return self.get_companies_with_matches_summary() 